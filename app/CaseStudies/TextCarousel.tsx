...
const TextCarousel: React.FC<TextCarouselProps> = ({ stages }) => {
    ...
    return (
        <div className="relative min-h-[200px] h-full w-full rounded-lg bg-black overflow-hidden">
            <div
            ...
            >
                {stages.map((stage, index) => (
                    <div
                        key={index}
                        className="flex-shrink-0 min-h-[200px] h-full flex flex-col items-center justify-center p-6"
                    >
                        <h4 className="text-cyan-400 text-xl sm:text-2xl">Stage {index + 1}</h4>
                        <p className="text-white text-light lg:text-xl text-center ">{stage}</p>
                    </div>
                ))}
            </div>
            <button
