...
const NavButton = ({ icon, href, ariaLabel, isActive }: { icon: JSX.Element; href: string; ariaLabel: string; isActive?: boolean }) => {
    const [showTooltip, setShowTooltip] = useState(false);

    const handleClick = () => {
        if (ariaLabel === "Go Back") {
            window.location.href = href; // Open in the same tab
        } else {
    ...
    return (
        ...
        >
            ...
            {/* Tooltip */}
            {showTooltip && (
                <span className="absolute bottom-[-3rem] mb-2 whitespace-nowrap text-lg bg-[#191b2a] shadow-[0_0_12px_-12px_rgba(0,0,0,0.8)] rounded-md px-2 py-1 z-10 flex items-center">
                    {ariaLabel}
                    {ariaLabel === "Live Demo" && (
                        <span
                            className={`ml-1 w-3 h-3 rounded-full ${isActive ? 'bg-[#03fbab]' : 'bg-[#fb4903]'}`}
                            style={{ display: 'block' }}
                        />
                    )}
                </span>
...
export function NavMenu({ srclink, livelink, back, isActive }: { srclink: string; livelink: string; back: string; isActive: boolean }) {
    ...
    return (
        ...
        >
            ...
            <NavButton icon={<Icons.globe />} href={livelink} ariaLabel="Live Demo" isActive={isActive} />
...
//         >
//             <NavButton icon={<Icons.github />} href={srclink} ariaLabel="Source Code" />
//             <NavButton icon={<Icons.globe />} href={livelink} ariaLabel="Live Demo" />< className="** make a  green dot  if Active value in rojectdata.sjoin is true, but if value is false , show a red dot next to the Live Demo text"/>
//             <NavButton icon={<Icons.back />} href={back} ariaLabel="Go Back" />
//         </div>
//     );
// };
